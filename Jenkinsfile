	...
	node('Windows') {
		...
		stage 'Build'
			...
			bat '''
				...
				7z a FactoryData_v01_%GIT_BRANCH%_%BUILD_NUMBER%.zip FactoryData/
				...
				7z a MailQueueService_v01_%GIT_BRANCH%_%BUILD_NUMBER%.zip MailQueueService/
				...
				7z a MessagePump_v01_%GIT_BRANCH%_%BUILD_NUMBER%.zip MessagePump/
				...
				7z a OCRService_v01_%GIT_BRANCH%_%BUILD_NUMBER%.zip OCRService/

				dotnet publish .\\Abiomed.RLR\\Abiomed.RLR.csproj -c Release /p:PublishDir="%WORKSPACE%\\RLR"
				7z a RLR_v01_%GIT_BRANCH%_%BUILD_NUMBER%.zip RLR/
			'''
